...
class mlp_policyfunction(torch.nn.Module):
	...
	def forward(self, qfunction_input, dimension_index):
		...
		input = torch.cat(
			(qfunction_input, 
			torch.tensor(dimension_index_normalize).float().view(1)),
			dim = -1
		)
	...
	def train(self):
		...
		for period in range(self.PERIOD_BOUND):
			...
			for step in range(self.DSE_action_space.get_lenth()): 
				...
				entropy, action, log_prob_sampled = self.actor.action_choose(self.policyfunction, self.DSE_action_space, step)
...
def run(iindex):
	...
	name = "record/objectvalue/" + "REINFORCE" + "_"+"gem5"+"_"+ str(
