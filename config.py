...
class test_config():
	def __init__(self, atype):
		#### step1 define model
		if(atype == 0): self.nnmodel, self.target = "VGG16", "cloud"
		elif(atype == 1): self.nnmodel, self.target = "VGG16", "normal"
		elif(atype == 2): self.nnmodel, self.target = "VGG16", "embed"
		elif(atype == 3): self.nnmodel, self.target = "MobilenetV3", "cloud"
		elif(atype == 4): self.nnmodel, self.target = "MobilenetV3", "normal"
		elif(atype == 5): self.nnmodel, self.target = "MobilenetV3", "embed"
		...
		BW = constraint(name = "BW", threshold = self.BW_THRESHOLD, threshold_ratio = self.THRESHOLD_RATIO)
		BRAM = constraint(name = "BRAM", threshold = self.BRAM_THRESHOLD, threshold_ratio = self.THRESHOLD_RATIO)
