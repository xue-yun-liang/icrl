...
class evaluation_function():
    def __init__(self, target):
        ...
        infile_network = "network .txt"
        infile_accelerator = "accelerator_mapping.txt"	
    ...
    def update_parameter(self, status, has_memory = False):
        # TODO: reconsturct the parameter class that we can automativly 
        # update those parameters from design_space
        function_test = False
        if(function_test):
            self.PE_num = status["PE_num"]
            self.PE_size = status["PE_size"]
            self.f = status["f"] * 1E+08
            self.minibatch = status["minibatch"]
            self.bitwidth = 2**status["bitwidth"] * 8 #### "0" for bit8, "1" for bit16, "2" for bit32
            assert((self.bitwidth == 8) or (self.bitwidth == 16) or (self.bitwidth == 32))
            self.scheme_chosen = list()
    ...
    def compute_scheme_chosen(self):
        ...
        for i in range(0,self.LN):
            ...
            IO_scheme1_L.append(datain_scheme1[i]+dataout_scheme1[i]+dataweight_scheme1[i])
            ...
            if self.layer_type[i]==1:#pooling
                ...
                dataout_scheme2_psumout.append(self.S[i]**2*self.N_in[i]*self.N_out[i]*2)
          
            IO_scheme2_L.append(datain_scheme2[i]+dataout_scheme2[i]+dataweight_scheme2[i])
            IO_scheme2_psumout_L.append(datain_scheme2[i]+dataout_scheme2_psumout[i]+dataweight_scheme2[i])
            ...
            IO_scheme3_L.append(datain_scheme3[i]+dataout_scheme3[i]+dataweight_scheme3[i])
            IO_scheme3_weightin_L.append(datain_scheme3[i]+dataout_scheme3[i]+dataweight_scheme3_weightin[i])
            ##########scheme1 BRAM###############################

            if self.layer_type[i]==0:#conv
                BRAM_cpt_req_scheme1_L.append(((self.R[i]*(self.K[i]+1)*2+self.K[i]**2*2)*self.PE_num+self.S[i]**2)*self.bitwidth/1e6)
            elif self.layer_type[i]==1: #pooling
                BRAM_cpt_req_scheme1_L.append((self.R[i]*4*(self.PE_num+1)+self.S[i]*self.PE_num)*self.bitwidth/1e6)
            else:
                BRAM_cpt_req_scheme1_L.append((self.datain_port_num*2)*self.bitwidth/1e6)
          
            if self.BN[i]==1:
                BRAM_BN_req_scheme1_L.append((self.S[i]**2*4/self.minibatch)*self.bitwidth/1e6)
            else:
                BRAM_BN_req_scheme1_L.append(0)
          
            BRAM_req_scheme1_L.append(BRAM_cpt_req_scheme1_L[i]+BRAM_BN_req_scheme1_L[i])

            ##########scheme2 BRAM###############################

            if self.layer_type[i]==0:#conv
                BRAM_cpt_req_scheme2_L.append((self.R[i]**2*2+self.K[i]**2*2*self.PE_num+self.S[i]**2*self.N_out[i])*self.bitwidth/1e6)
                BRAM_cpt_req_scheme2_psumout_L.append((self.R[i]**2*2+self.K[i]**2*2*self.PE_num+self.S[i]*self.N_out[i])*self.bitwidth/1e6)
            elif self.layer_type[i]==1: #pooling
                BRAM_cpt_req_scheme2_L.append((self.R[i]*4*(self.PE_num+1)+self.S[i]*self.PE_num)*self.bitwidth/1e6)
                BRAM_cpt_req_scheme2_psumout_L.append((self.R[i]*4*(self.PE_num+1)+self.S[i]*self.PE_num)*self.bitwidth/1e6)
            else:#FC
                BRAM_cpt_req_scheme2_L.append((self.datain_port_num*2)*self.bitwidth/1e6)
                BRAM_cpt_req_scheme2_psumout_L.append((self.datain_port_num*2)*self.bitwidth/1e6)
          
            if self.BN[i]==1:
                BRAM_BN_req_scheme2_L.append((self.S[i]**2*4/self.minibatch)*self.bitwidth/1e6)
            else:
                BRAM_BN_req_scheme2_L.append(0)
          
            BRAM_req_scheme2_L.append(BRAM_cpt_req_scheme2_L[i]+BRAM_BN_req_scheme2_L[i])
            BRAM_req_scheme2_psumout_L.append(BRAM_cpt_req_scheme2_psumout_L[i]+BRAM_BN_req_scheme2_L[i])
         
         ##########scheme3 BRAM###############################

            if self.layer_type[i]==0:#conv
                BRAM_cpt_req_scheme3_L.append((self.R[i]*(self.K[i]+1)*2*self.N_in[i]+self.K[i]**2*2*self.N_in[i]+self.S[i]*1)*self.bitwidth/1e6)
                BRAM_cpt_req_scheme3_weightin_L.append((self.R[i]*(self.K[i]+1)*2*self.N_in[i]+self.K[i]**2*self.N_out[i]*self.N_in[i]+self.S[i]*1)*self.bitwidth/1e6)
            elif self.layer_type[i]==1: #pooling
                BRAM_cpt_req_scheme3_L.append((self.R[i]*4*(self.PE_num+1)+self.S[i]*self.PE_num)*self.bitwidth/1e6)
                BRAM_cpt_req_scheme3_weightin_L.append((self.R[i]*4*(self.PE_num+1)+self.S[i]*self.PE_num)*self.bitwidth/1e6)
            else:#FC
                BRAM_cpt_req_scheme3_L.append((self.datain_port_num*2)*self.bitwidth/1e6)
                BRAM_cpt_req_scheme3_weightin_L.append((self.datain_port_num*2)*self.bitwidth/1e6)
          
            if self.BN[i]==1:
                BRAM_BN_req_scheme3_L.append((self.S[i]**2*4/self.minibatch)*self.bitwidth/1e6)
            else:
                BRAM_BN_req_scheme3_L.append(0)
          
            BRAM_req_scheme3_L.append(BRAM_cpt_req_scheme3_L[i]+BRAM_BN_req_scheme3_L[i])
            BRAM_req_scheme3_weightin_L.append(BRAM_cpt_req_scheme3_weightin_L[i]+BRAM_BN_req_scheme3_L[i])
        ...
        for i in range(0,self.LN):
            ...
            elif IO_scheme1_L[i]<IO_scheme2_L[i] and IO_scheme1_L[i]<IO_scheme3_L[i]: #scheme1 IO best
                #scheme_chosen.append(1)
                scheme2_psumout.append(0)
                scheme3_weightin.append(0)
          
            elif IO_scheme2_L[i]<IO_scheme1_L[i] and IO_scheme2_L[i]<IO_scheme3_L[i]: #scheme2 IO best 
                if BRAM_req_scheme2_L[i]<self.BRAM_total:  #&& scheme2 BRAM enough
                    #scheme_chosen.append(2)
                    scheme2_psumout.append(0)
                    scheme3_weightin.append(0)
           
                elif IO_scheme2_psumout_L[i]<IO_scheme1_L[i] and IO_scheme2_psumout_L[i]<IO_scheme3_L[i]: #scheme2 psumout IO best
                    #scheme_chosen.append(2)
    ...
    def runtime(self):
        ...
        for i in range(0,self.LN):
            ...
            if self.layer_type[i]==0:#conv
                if self.scheme_chosen[i]==1:
                    t_L_c=math.ceil(self.N_in[i]/self.PE_num)*self.N_out[i]*(self.S[i]**2)*self.K[i]**2/(self.PE_size**2)/self.f
                else:
                    t_L_c=math.ceil(self.N_out[i]/self.PE_num)*self.N_in[i]*(self.S[i]**2)*self.K[i]**2/(self.PE_size**2)/self.f
                ...
                t_L_c=math.ceil(self.N_out[i]/self.PE_num)*self.N_in[i]*((self.R[i]/2)**2)*self.K[i]**2/(self.PE_size**2)/self.f

            #### IO time
            if self.BN[i]==1:
                IO = ((datain[i]+dataout[i]+dataweight[i])+(self.S[i]*4/self.minibatch))*self.bitwidth
            else:
    ...
    def power(self):
        ...
        for i in range(0,self.LN):
            ...
            Energy_BRAM_L.append(self.N_in[i]*self.S[i]*self.R[i]*self.K[i]*self.Emem_B+self.S[i]*self.S[i]*self.N_out[i]*self.Emem_B*self.S[i])
        
            #computation
            Energy_COMP_L.append(self.S[i]**2*self.N_in[i]*self.K[i]**2*self.N_out[i]*self.Ecpt)
        
            #BN
            if self.BN[i]==1:
                Energy_BN_L.append(self.N_out[i]/self.minibatch*2*self.Emem_D+self.N_out[i]*self.S[i]**2*self.Ecpt)
            else:
    ...
    def bandwidth(self):
        ...
        for i in range(0,self.LN):
            ...
            Bandwidth_cpt_req_L.append((datain[i]+dataout[i]+dataweight[i])*self.bitwidth/t_L[i]/1e9)

            if self.BN[i]==1:
                Bandwidth_BN_req_L.append(self.S[i]*4/self.minibatch*self.bitwidth/t_L[i]/1e9)
            else:
