...
#+++++++++++++++++++++++++++++++++++++++++second++++++++++++++++++++++++++++++++++
	...
	def train(self):
		...
		next_status = dict() #S' 
		reward_log_name = "record/objectvalue_double/" + "reward_" + "ERDSE" + "_" + str(self.iindex) + ".txt"
		...
		for period in range(period_bound):
			...
			for step in range(self.DSE_action_space.get_lenth()): 
				...
				action, probs_noise = self.actor.action_choose_with_no_grad_2(self.policyfunction,self.policyfunction_2, self.DSE_action_space, step,w1,w2)
				...
				if(step < (self.DSE_action_space.get_lenth() - 1)): #delay reward, only in last step the reward will be asigned
					...
					if(period < self.SAMPLE_PERIOD_BOUND):
						...
						if(objectvalue < self.best_objectvalue and objectvalue2 < self.best_objectvalue2 and self.constraints.is_all_meet()):
				...
				for _ in range(self.BATCH_SIZE):
					...
					for t in range(T2):
						s_entropy2, s_log_prob2 = get_log_prob(self.policyfunction_2, self.DSE_action_space,
															 s_status_list2[t], s_action_list2[t], s_step_list2[t])
...
def run(iindex):
	...
	name = "record/objectvalue_double/" + "ERDSE" + "_" +"dobule"+ str(iindex) + ".xls"
	workbook.save(name)
	...
	name = "record/objectvalue_double/"  + "ERDSE" + "_" +"dobule"+str(
