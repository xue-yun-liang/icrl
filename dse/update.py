...
def update_archive(
    in_, fitness_, archive_in, archive_fitness, thresh, mesh_div, min_, max_, particals
):
    ...
    if (curr_archiving_in).shape[0] > thresh:
        clear_ = archive.clear_archiving(
            curr_archiving_in, curr_archiving_fit, mesh_div, min_, max_, particals
        )
...
def update_gbest(archiving_in, archiving_fit, mesh_div, min_, max_, particals):
    get_g = archive.get_gbest(
        archiving_in, archiving_fit, mesh_div, min_, max_, particals
    )
