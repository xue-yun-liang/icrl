...
class RLDSE():
	def __init__(self, iindex):
		...
		self.policyfunction_2 = mlp_policyfunction(self.DSE_action_space.get_lenth(), action_scale_list)
		self.policyfunction_3 = mlp_policyfunction(self.DSE_action_space.get_lenth(), action_scale_list)
...
# ++++++++++++++++++++++++++fillter+++++++++++++++++++++++++++++
	...
	def train(self):
		...
		next_status = dict() #S' 
		reward_log_name = "record/objectvalue_double_three_policy/" + "reward_" + str(self.iindex) + ".txt"
		...
		while(period < self.SAMPLE_PERIOD_BOUND + self.PERIOD_BOUND):
			...
			for step in range(self.DSE_action_space.get_lenth()): 
				...
				action, probs_noise = self.actor.action_choose_with_no_grad_3(self.policyfunction,self.policyfunction_2,self.policyfunction_3, self.DSE_action_space, step,signol)
			...
			self.fillter_train_flag = ((period - self.SAMPLE_PERIOD_BOUND) % 50 == 0) and (
					period - self.SAMPLE_PERIOD_BOUND != 0)

			if (period >= self.SAMPLE_PERIOD_BOUND and self.fillter_train_flag):
				print(f"**************  Training the fillter 1    ******************")
				self.train_fillter(self.fillter, self.fillter_obs_buffer, self.fillter_reward_buffer)
			...
			if (period >= self.SAMPLE_PERIOD_BOUND and self.fillter_train_flag):
				...
				self.train_fillter(self.fillter2, self.fillter_obs_buffer2, self.fillter_reward_buffer2)
			obs = self.DSE_action_space.get_obs()
			t_obs = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(0)
			self.fillter2.eval()
			...
			if (period >= self.SAMPLE_PERIOD_BOUND and self.fillter_train_flag):
				...
				self.train_fillter(self.fillter3, self.fillter_obs_buffer3, self.fillter_reward_buffer3)
			obs = self.DSE_action_space.get_obs()
			t_obs = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(0)
			self.fillter3.eval()
			...
			if (metrics != None):
				...
				reward_runtime = 6.25/ (runtime /0.004028 * self.constraints.get_punishment())
				reward_power = 4 / (power/44.257917 * self.constraints.get_punishment())
				objectvalue = runtime
				objectvalue2 = power
			...
			if(period < self.SAMPLE_PERIOD_BOUND):
				...
				if(objectvalue2 < self.best_objectvalue2 and self.constraints.is_all_meet()):
			...
			sample = {"reward": reward,"reward2": reward2,"reward3": reward3, "return_list": return_list, "status_list": status_list,
			...
			if (len(self.replay_buffer) < self.BATCH_SIZE):
				...
				elif((sample["reward"] == min_sample["reward"])and(sample["reward3"] > min_sample["reward3"])):
					index = self.replay_buffer.index(min_sample)
					self.replay_buffer[index] = sample
		#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++second++++++++++++++++++++++++++++++++++++
			...
			if (len(self.replay_buffer2) < self.BATCH_SIZE):
				...
				min_sample = min(self.replay_buffer2, key=lambda sample: sample["reward2"])
		...
		# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++third++++++++++++++++++++++++++++++++++++
			...
			if (len(self.replay_buffer3) < self.BATCH_SIZE):
				...
				min_sample = min(self.replay_buffer3, key=lambda sample: sample["reward3"])
				if (sample["reward3"] > min_sample["reward3"]):
					index = self.replay_buffer3.index(min_sample)
					self.replay_buffer3[index] = sample
				elif((sample["reward3"] == min_sample["reward3"])and(sample["reward"] > min_sample["reward"])):
					index = self.replay_buffer3.index(min_sample)
					self.replay_buffer3[index] = sample
			...
			if(period < self.SAMPLE_PERIOD_BOUND):
				...
				for _ in range(self.BATCH_SIZE):
					...
					pi_noise = torch.tensor(1)
					for t in range(T2):
						s_entropy2, s_log_prob2 = get_log_prob(self.policyfunction_2, self.DSE_action_space,
															 s_status_list2[t], s_action_list2[t], t)
						retrun_item2 = -1 * s_log_prob2 * (s_return_list2[t] - self.BASE_LINE)
				...
				for _ in range(self.BATCH_SIZE):
					...
					pi_noise = torch.tensor(1)
					for t in range(T3):
						s_entropy3, s_log_prob3 = get_log_prob(self.policyfunction_3, self.DSE_action_space,
															   s_status_list3[t], s_action_list3[t], t)
						retrun_item3 = -1 * s_log_prob3 * (s_return_list3[t] - self.BASE_LINE)
...
def run(iindex):
	...
	name = "record/objectvalue_double_three_policy/" + "_" + "MOMPRDSE" + "_"+"runtime"+str(iindex) +"500"+ ".xls"
	workbook.save(name)
	...
	for index, best_objectvalue in enumerate(DSE.best_objectvalue2_list):
		worksheet.write(index+1, 0, index+1)
		worksheet.write(index+1, 1, best_objectvalue)
	name = "record/objectvalue_double_three_policy/" + "_" + "MOMPRDSE" + "_"+"power"+str(iindex) +"500"+ ".xls"
	workbook.save(name)
	...
	name = "record/objectvalue_double_three_policy/" + "_" + "MOMPRDSE" + "_"+ str(
		iindex) +"all_value"+"500"+".xls"
	workbook2.save(name)
