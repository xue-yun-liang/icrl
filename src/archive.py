...
class mesh_crowd(object):
    def __init__(
        self, curr_archiving_in, curr_archiving_fit, mesh_div, min_, max_, particals
    ):
        ...
        self.crowd_archiving = np.zeros(
            (self.num_)
        )  # 拥挤度矩阵，用于记录当前网格的粒子数量
        self.gbest_in = np.zeros(
            (self.particals, self.curr_archiving_in.shape[1])
        )  # 初始化gbest坐标
        self.gbest_fit = np.zeros(
            (self.particals, self.curr_archiving_fit.shape[1])
        )  # 初始化gbest适应度ֵ
        self.min_ = min_
        self.max_ = max_
    ...
    def cal_mesh_id(self, in_):
        ...
        for i in range(self.curr_archiving_in.shape[1]):
            id_dim = int(
                (in_[i] - self.min_[i]) * self.mesh_div / (self.max_[i] - self.min_[i])
            )  # self.num_
    ...
    def get_crowd(self):
        index_ = (
            np.linspace(0, self.num_ - 1, self.num_)
        ).tolist()  # 定义一个数组存放粒子集的索引号，用于辅助计算
...
class get_gbest(mesh_crowd):
    def __init__(
        self, curr_archiving_in, curr_archiving_fit, mesh_div_num, min_, max_, particals
    ):
        super(get_gbest, self).__init__(
            curr_archiving_in, curr_archiving_fit, mesh_div_num, min_, max_, particals
        )
    ...
    def get_probability(self):
        ...
        self.probability_archiving = self.probability_archiving / np.sum(
            self.probability_archiving
        )  # 归一化，粒子被选择概率为1

    def get_gbest_index(self):
        random_pro = random.uniform(
            0.0, 1.0
        )  # 有点像轮盘赌，概率越高（拥挤度小），被选择的概率越大
        for i in range(self.num_):
...
class clear_archiving(get_gbest):
    ...
    def get_probability(self):
        for i in range(self.num_):
            self.probability_archiving = self.crowd_archiving / sum(
                self.crowd_archiving
            )

    def get_clear_index(self):  # 按概率清除粒子，拥挤度高的粒子被清除的概率越高
        len_clear = (self.curr_archiving_in).shape[
            0
        ] - self.thresh  # �#需要清除掉的粒子数量
        clear_index = []
        while len(clear_index) < len_clear:
            random_pro = random.uniform(
                0.0, np.sum(self.probability_archiving)
            )  # 生成0-1的随机数
            for i in range(self.num_):
                if random_pro <= np.sum(self.probability_archiving[0 : i + 1]):
    ...
    def clear_(self, thresh):
        ...
        # gbest_index=self.get_gbest_index()
        self.curr_archiving_in = np.delete(
            self.curr_archiving_in, clear_index, axis=0
        )  # 删除存档中部分粒子
        self.curr_archiving_fit = np.delete(
            self.curr_archiving_fit, clear_index, axis=0
        )  # 删除存档中粒子适应度ֵ
